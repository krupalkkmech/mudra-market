...
import { ICoinDetails, ICurrency } from "@/types/crypto";
import { CURRENCIE_MAP } from "@/utils/constants";

interface CoinDetailsClientProps {
  initialCoinData: ICoinDetails;
}

const CoinDetailsClient: React.FC<CoinDetailsClientProps> = ({
  initialCoinData,
}) => {
  const dispatch = useDispatch();
  const { coins, currency } = useSelector((state: RootState) => state.crypto);
  const [coinCurrency, setCoinCurrency] = useState<ICurrency>(currency);
  const [selectedCoinData, setSelectedCoinDataState] =
    useState(initialCoinData);

  useEffect(() => {
    dispatch(setSelectedCoinData(initialCoinData));
    setSelectedCoinDataState(initialCoinData);

    const coinData = coins.find(
      (eachCoin) => eachCoin?.id === initialCoinData.id
    );
    if (coinData) {
  ...
  }, [dispatch, initialCoinData, coins]);
...
export default CoinDetailsClient;
